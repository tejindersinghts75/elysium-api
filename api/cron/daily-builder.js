  ...
  try {
    ...
    for (const data of users) {
      ...
      try {
        ...
        const user = userData[userEntryKey];

        // ⛔ STOP cron if refund requested or completed
        if (
          user?.allRefund === true ||
          user?.backerPayment?.refundStatus === "processing" ||
          user?.backerPayment?.refundStatus === "success"
        ) {
          console.log(`⛔ Skipping refunded user ${userEntryKey}`);
          continue;
        }
